...
void gengine::interface::network::system::ClientEventPublisher<Events...>::init(void) {
    ...
    this->template subscribeToEvent<gengine::interface::event::ItsMe>(&ClientEventPublisher::setMe);
...
template <class... Events>
void gengine::interface::network::system::ClientEventPublisher<Events...>::setMe(gengine::interface::event::ItsMe &e) {
    m_me = e.myUUID;
}
...
void gengine::interface::network::system::ClientEventPublisher<Events...>::dynamicSubscribe(void) {
    ...
    this->template subscribeToEvent<T>([this](T &event) -> void {
        ...
        m_eventCount++;
        if (!m_me.is_nil()) {
            gengine::interface::event::SharedEvent<T> sharedEvent(event, m_me);
            this->template publishEvent<gengine::interface::event::SharedEvent<T>>(sharedEvent);
        }
    });
    m_id++;
}
